...
class AppCommands(DeviceCommands):
    ...
    def openCatalog(self, catalogName):
        ...
        if not match[0][1]:
            log.error('No match color failed on opening the catalog {0} {1}'.format(catalogName, match))
            return False
    
        log.info('No match color successful on opening the catalog {0} {1}'.format(catalogName, match))
        return True
